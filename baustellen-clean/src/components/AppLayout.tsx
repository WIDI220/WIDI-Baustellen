...
import { LayoutDashboard, HardHat, Clock, Package, FileText, Camera, AlertTriangle, LogOut, ChevronLeft, ChevronRight, ChevronDown, FileUp, Users, Home, Archive, Zap, Building2 } from 'lucide-react';
...
// Status-Sortierreihenfolge und Farben
...
function GewerkSection({ label, icon, accentColor, accentBg, items, collapsed, sidebarCollapsed, currentPath, onNavigate, onToggle }: GewerkSectionProps) {
  ...
  return (
    <div style={{ marginBottom: 4 }}>
      ...
      >
        ...
        <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', color: accentColor, flex: 1 }}>
          {label}
        </span>
        <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 5px', borderRadius: 99, background: accentBg, color: accentColor }}>
          {items.length}
        </span>
        <ChevronDown
          size={10}
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderRadius: 8,
                  cursor: 'pointer',
                  background: isActive ? st.bg : 'rgba(255,255,255,0.04)',
                  border: isActive ? `1px solid ${st.border}` : '1px solid transparent',
                  transition: 'all .15s',
                }}
                onMouseEnter={e => {
                  if (!isActive) {
                    (e.currentTarget as HTMLElement).style.background = st.bg;
                    (e.currentTarget as HTMLElement).style.border = `1px solid ${st.border}`;
                  }
                }}
                onMouseLeave={e => {
                  if (!isActive) {
              ...
              >
                ...
                <div style={{
                  ...
                  boxShadow: isActive ? `0 0 4px ${st.dot}` : 'none',
                }} />
                {/* Name */}
                <span style={{
                  fontSize: 11,
                  color: isActive ? st.text : 'rgba(255,255,255,0.45)',
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  // Einklapp-State für Gewerk-Gruppen — in localStorage speichern
  ...
  const toggleHochbau = () => {
    const next = !hochbauCollapsed;
    setHochbauCollapsed(next);
    try { localStorage.setItem('sidebar_hochbau_collapsed', String(next)); } catch {}
  };
  const toggleElektro = () => {
    const next = !elektroCollapsed;
    setElektroCollapsed(next);
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#f8fafc', fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <aside style={{
        ...
        {/* Logo */}
        <div style={{ padding: sidebarCollapsed ? '18px 0' : '18px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 10, justifyContent: sidebarCollapsed ? 'center' : 'flex-start' }}>
        ...
        <div style={{ padding: '8px 8px 2px' }}>
          <button onClick={() => navigate('/')}
            style={{ display: 'flex', alignItems: 'center', gap: sidebarCollapsed ? 0 : 8, justifyContent: sidebarCollapsed ? 'center' : 'flex-start', width: '100%', padding: sidebarCollapsed ? '9px 0' : '7px 12px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, color: 'rgba(255,255,255,0.5)', fontSize: 11, fontWeight: 500, cursor: 'pointer', transition: 'all .15s' }}
        ...
        <nav style={{ flex: 1, padding: '4px 8px', display: 'flex', flexDirection: 'column', gap: 2, overflowY: 'auto' }}>
          ...
          <GewerkSection
            ...
            items={hochbau}
            collapsed={hochbauCollapsed}
            sidebarCollapsed={sidebarCollapsed}
            currentPath={location.pathname}
            onNavigate={(id) => navigate(`/baustellen/liste/${id}`)}
            onToggle={toggleHochbau}
          />
          ...
          <GewerkSection
            ...
            items={elektro}
            collapsed={elektroCollapsed}
            sidebarCollapsed={sidebarCollapsed}
            currentPath={location.pathname}
            onNavigate={(id) => navigate(`/baustellen/liste/${id}`)}
            onToggle={toggleElektro}
          />

          {/* Rest der Nav-Items */}
          {!sidebarCollapsed && (
            <p style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '.08em', color: 'rgba(255,255,255,0.2)', fontWeight: 600, padding: '8px 8px 4px', margin: 0, marginTop: 4 }}>Werkzeuge</p>
          )}
          {NAV.slice(1).map(({ to, icon: Icon, label }) => {
            const active = location.pathname === to || location.pathname.startsWith(to + '/');
            return (
              <NavLink key={to} to={to} className={`nav-item ${active ? 'active' : ''}`}
                style={{ display: 'flex', alignItems: 'center', gap: sidebarCollapsed ? 0 : 10, padding: sidebarCollapsed ? '10px 0' : '8px 12px', justifyContent: sidebarCollapsed ? 'center' : 'flex-start', borderRadius: 10, textDecoration: 'none', fontSize: 13, fontWeight: 500, color: active ? '#fff' : 'rgba(255,255,255,0.45)', borderLeft: active ? `3px solid ${ACCENT}` : '3px solid transparent', position: 'relative' }}>
                <Icon size={15} style={{ flexShrink: 0 }} />
        ...
        <div style={{ padding: '8px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', gap: 2 }}>
          <button onClick={() => setSidebarCollapsed(!sidebarCollapsed)} className="collapse-btn"
            style={{ display: 'flex', alignItems: 'center', gap: sidebarCollapsed ? 0 : 8, justifyContent: sidebarCollapsed ? 'center' : 'flex-start', width: '100%', padding: sidebarCollapsed ? '10px 0' : '8px 12px', background: 'none', border: 'none', borderRadius: 10, color: 'rgba(255,255,255,0.35)', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>
            {sidebarCollapsed ? <ChevronRight size={14} /> : <><ChevronLeft size={14} /><span>Einklappen</span></>}
          </button>
          <button onClick={signOut} className="collapse-btn"
            style={{ display: 'flex', alignItems: 'center', gap: sidebarCollapsed ? 0 : 8, justifyContent: sidebarCollapsed ? 'center' : 'flex-start', width: '100%', padding: sidebarCollapsed ? '10px 0' : '8px 12px', background: 'none', border: 'none', borderRadius: 10, color: 'rgba(255,255,255,0.3)', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}
