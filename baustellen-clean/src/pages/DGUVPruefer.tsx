...
interface Pruefer {
  id: string; name: string; kuerzel: string; csv_name: string;
  soll_monat: number; aktiv: boolean;
}

const EMPTY: Omit<Pruefer, 'id'> = { name: '', kuerzel: '', csv_name: '', soll_monat: 0, aktiv: true };
...
export default function DGUVPruefer() {
  ...
  const { data: pruefer = [] } = useQuery({
    ...
    queryFn: async () => { const { data } = await supabase.from('dguv_pruefer').select('*').order('name'); return data ?? []; }
  });

  const upsert = useMutation({
    mutationFn: async () => {
      if (!form.name.trim()) throw new Error('Name erforderlich');
      const payload = { name: form.name.trim(), kuerzel: form.kuerzel.trim().toUpperCase(), csv_name: form.csv_name.trim() || form.name.trim(), soll_monat: Number(form.soll_monat) || 0, aktiv: form.aktiv };
      if (editId) { const { error } = await supabase.from('dguv_pruefer').update(payload).eq('id', editId); if (error) throw error; }
      else { const { error } = await supabase.from('dguv_pruefer').insert(payload); if (error) throw error; }
    },
    onSuccess: () => { toast.success(editId ? 'Prüfer aktualisiert' : 'Prüfer angelegt'); qc.invalidateQueries({ queryKey: ['dguv-pruefer'] }); reset(); },
    onError: (e: any) => toast.error(e.message),
  });

  const del = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from('dguv_pruefer').delete().eq('id', id); if (error) throw error; },
  ...
  function startEdit(p: Pruefer) { setEditId(p.id); setForm({ name: p.name, kuerzel: p.kuerzel, csv_name: p.csv_name, soll_monat: p.soll_monat, aktiv: p.aktiv }); setShowForm(true); }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, paddingBottom: 32, fontFamily: "'Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#94a3b8', margin: '4px 0 0' }}>{aktivCount} aktive Prüfer · Soll gesamt {gesamtSoll.toLocaleString('de-DE')} Messungen/Monat</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
        {[
          ...
          <div key={i} style={{ background: k.bg, border: `1px solid ${k.border}`, borderRadius: 16, padding: '18px 20px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ width: 36, height: 36, background: '#fff', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
      ...
      {showForm && (
        <div style={{ background: '#fff', borderRadius: 18, border: '1.5px solid #f59e0b', padding: '20px 24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 2fr 1fr', gap: 12, marginBottom: 14 }}>
            {[
              { label: 'Vollständiger Name *', key: 'name', placeholder: 'Marcel Münch' },
              { label: 'Kürzel', key: 'kuerzel', placeholder: 'MM' },
              { label: 'Name in CSV-Datei', key: 'csv_name', placeholder: 'M. Münch' },
          ...
          <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#fafafa', borderBottom: '1px solid #f1f5f9' }}>
                {['Name', 'Kürzel', 'CSV-Name', 'Soll/Monat', 'Status', ''].map(h => (
            ...
            <tbody>
              {p.map(pr => (
                <tr key={pr.id} style={{ borderBottom: '1px solid #f8fafc' }}>
                  ...
                  <td style={{ padding: '13px 16px', color: '#64748b', fontStyle: 'italic' }}>{pr.csv_name}</td>
                  <td style={{ padding: '13px 16px' }}>
                    <span style={{ fontWeight: 700, fontSize: 14, color: '#f59e0b' }}>{pr.soll_monat.toLocaleString('de-DE')}</span>
