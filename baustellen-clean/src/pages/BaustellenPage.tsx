...
import { Plus, Search, HardHat, ArrowRight, Pencil, Trash2, Calendar, TrendingUp, AlertTriangle, Lightbulb, Zap, Building2, PauseCircle } from 'lucide-react';
...
function BaustelleKarte({ b, sw, mat, nach, onEdit, onDelete, navigate }: any) {
  ...
  // Deadline-Warning NUR wenn nicht "nicht_gestartet"
  ...
  return (
    <div
      style={{ background:'#fff', borderRadius:16, border:'1px solid #f1f5f9', padding:'18px 20px', cursor:'pointer', position:'relative', overflow:'hidden', borderLeft:`3px solid ${st.dot}`, transition:'box-shadow .2s, transform .2s' }}
    ...
    >
      <div style={{ display:'flex', alignItems:'flex-start', gap:14 }}>
        ...
        <div style={{ width:42, height:42, borderRadius:13, background:`${st.dot}15`, border:`1px solid ${st.dot}25`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
          {b.gewerk === 'Elektro' ? <Zap size={20} style={{ color:st.dot }} /> : <HardHat size={20} style={{ color:st.dot }} />}
        </div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:4, flexShrink:0 }} onClick={e => e.stopPropagation()}>
          ...
          <button onClick={e => { e.stopPropagation(); if (confirm(`"${b.name}" löschen?`)) onDelete(b.id); }}
...
// Sektion-Header
function SektionHeader({ icon, label, count, color, bg }: any) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 16px', background:bg, borderRadius:12, marginBottom:8, border:`1px solid ${color}20` }}>
      ...
      <span style={{ fontSize:11, fontWeight:700, padding:'2px 8px', borderRadius:99, background:`${color}20`, color }}>{count} aktiv</span>
    </div>
  );
}
...
export default function BaustellenPage() {
  ...
  // Aktive (nicht abgeschlossen/abgerechnet) filtern und suchen
  ...
  // Nach Gewerk aufteilen
  const hochbauBS       = filtered.filter(b => (b.gewerk === 'Hochbau' || b.gewerk === 'Beides') && b.status !== 'nicht_gestartet');
  const elektroBS       = filtered.filter(b => (b.gewerk === 'Elektro'  || b.gewerk === 'Beides') && b.status !== 'nicht_gestartet');
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:20, paddingBottom:32, fontFamily:"'Inter',system-ui,sans-serif" }}>
      ...
      {hochbauBS.length > 0 && (
        <div>
          <SektionHeader
            ...
            label="Hochbau"
            count={hochbauBS.length}
            color="#2563eb"
            bg="#eff6ff"
          />
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {hochbauBS.map(b => (
              <BaustelleKarte key={b.id} b={b} sw={sw} mat={mat} nach={nach}
                onEdit={openEdit} onDelete={(id:string) => del.mutate(id)} navigate={navigate} />
            ))}
          </div>
        </div>
      )}
      ...
      {nichtGestartetBS.length > 0 && (
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 16px', background:'#f8fafc', borderRadius:12, marginBottom:8, border:'1px solid #e2e8f020' }}>
            <PauseCircle size={14} style={{ color:'#94a3b8' }} />
            <span style={{ fontSize:13, fontWeight:700, color:'#64748b' }}>Nicht gestartet</span>
            <span style={{ fontSize:11, fontWeight:700, padding:'2px 8px', borderRadius:99, background:'#e2e8f0', color:'#64748b' }}>{nichtGestartetBS.length}</span>
            <span style={{ fontSize:11, color:'#94a3b8', marginLeft:4 }}>— keine Deadline-Warnungen aktiv</span>
          </div>
