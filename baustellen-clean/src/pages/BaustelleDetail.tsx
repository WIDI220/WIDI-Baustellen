    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-4">
        ...
        <Button variant="outline" size="sm" onClick={() => {
          ...
          const gesamtH   = sw.reduce((s: number, w: any) => s + Number(w.stunden ?? 0), 0);
          const vonWem    = [...new Set(sw.map((w: any) => w.employees?.name).filter(Boolean))].join(', ') || '_______________';
          const heute     = new Date().toLocaleDateString('de-DE');
          exportAbnahmescheinExcel({ aNummer, proj, leistungseinheit: 'Std.', menge: String(gesamtH), ausgefuehrtAm: heute, ausgefuehrtVon: vonWem });
        }} style={{borderColor:'#2563eb',color:'#1d4ed8',fontWeight:600}}>📋 Abnahmeschein Excel</Button>
      </div>
