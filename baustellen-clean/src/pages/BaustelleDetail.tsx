...
import { ArrowLeft, Euro, Clock, Package, FileText, Camera, Plus, Pencil, Trash2, Upload, TrendingUp } from 'lucide-react';
...
export default function BaustelleDetail() {
  ...
  const { data: bs, isLoading: bsLoading } = useQuery({
    ...
    queryFn: async () => { const { data, error } = await supabase.from('baustellen').select('*').eq('id', id).single(); if (error) throw error; return data; },
    enabled: !!id,
  });
  ...
  const { data: stunden = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('bs_stundeneintraege')
        .select('*, employees(id, name, kuerzel, stundensatz)')
        .eq('baustelle_id', id)
  ...
  const { data: fotos = [] } = useQuery({
    ...
    queryFn: async () => { const { data, error } = await supabase.from('bs_fotos').select('*, employees(name,kuerzel)').eq('baustelle_id', id).order('created_at', { ascending: false }); if (error) throw error; return data ?? []; },
    enabled: !!id,
  });
  ...
  useEffect(() => {
    if (!id) return;
    const channel = supabase.channel(`baustelle-${id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'bs_stundeneintraege', filter: `baustelle_id=eq.${id}` },
        () => queryClient.invalidateQueries({ queryKey: ['bs-stunden', id] }))
      .on('postgres_changes', { event: '*', schema: 'public', table: 'bs_materialien', filter: `baustelle_id=eq.${id}` },
        () => queryClient.invalidateQueries({ queryKey: ['bs-mat', id] }))
      .on('postgres_changes', { event: '*', schema: 'public', table: 'bs_nachtraege', filter: `baustelle_id=eq.${id}` },
        () => queryClient.invalidateQueries({ queryKey: ['bs-nach', id] }))
      .on('postgres_changes', { event: '*', schema: 'public', table: 'baustellen', filter: `id=eq.${id}` },
  ...
  // Budget + genehmigte Nachträge = effektives Budget
  ...
  const updateStatus = useMutation({
    ...
    onSuccess: () => { toast.success('Status aktualisiert'); queryClient.invalidateQueries({ queryKey: ['baustellen-list'] }); },
  });

  const saveStunden = useMutation({
    mutationFn: async () => {
      if (!stundenForm.mitarbeiter_id) throw new Error('Bitte Mitarbeiter auswählen');
      if (!stundenForm.stunden) throw new Error('Bitte Stunden eingeben');
      const payload = {
        baustelle_id: id,
        mitarbeiter_id: stundenForm.mitarbeiter_id,
        datum: stundenForm.datum,
        stunden: parseFloat(String(stundenForm.stunden).replace(',','.')),
        beschreibung: stundenForm.beschreibung || null,
      };
      if (editStunden) {
        const { error } = await supabase.from('bs_stundeneintraege').update(payload).eq('id', editStunden.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('bs_stundeneintraege').insert(payload);
        if (error) throw error;
      }
    },
    onSuccess: () => {
      toast.success('Stunden gespeichert');
      setStundenDialog(false);
      setEditStunden(null);
      setStundenForm(STUNDEN_EMPTY);
      queryClient.invalidateQueries({ queryKey: ['bs-stunden', id] });
    },
  ...
  const exportPDF = () => {
    ...
    doc.text(`Auftraggeber: ${bs.auftraggeber || '–'}  ·  Status: ${st?.label}`, 14, 32);
    doc.text(`Budget: ${fmtEur(budget)}  ·  Nachträge gen.: ${fmtEur(nachtragGenehmigt)}  ·  Kosten: ${fmtEur(gesamtkosten)}`, 14, 40);
    autoTable(doc, { startY: 50, head: [['Mitarbeiter','Datum','Stunden','Kosten','Beschreibung']], body: sw.map(w => [w.employees?.name||'–', w.datum, `${w.stunden}h`, fmtEur(Number(w.stunden)*Number(w.employees?.stundensatz??STUNDEN_SATZ)), w.beschreibung||'']) });
    const y1 = (doc as any).lastAutoTable.finalY + 8;
    doc.setFontSize(13); doc.setTextColor(0); doc.text('Material', 14, y1);
    autoTable(doc, { startY: y1+5, head: [['Bezeichnung','Menge','Einzelpreis','Gesamt','Status']], body: mat.map(m => [m.bezeichnung, `${m.menge} ${m.einheit}`, fmtEur(m.einzelpreis), fmtEur(m.gesamtpreis), m.status]) });
    const y2 = (doc as any).lastAutoTable.finalY + 8;
    doc.setFontSize(13); doc.text('Nachträge', 14, y2);
    autoTable(doc, { startY: y2+5, head: [['Titel','Betrag','Status']], body: nach.map(n => [n.titel, fmtEur(n.betrag), n.status]) });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: 'Budget', sub: nachtragGenehmigt > 0 ? `+${fmtEur(nachtragGenehmigt)} Nachträge` : null, value: fmtEur(effektivBudget), icon: Euro, color: 'text-[#1e3a5f]', bg: 'bg-[#1e3a5f]/5' },
          ...
          { label: 'Personalkosten', sub: `${Math.round(gesamtStunden * 10) / 10}h gesamt`, value: fmtEur(personalkosten), icon: Clock, color: 'text-purple-600', bg: 'bg-purple-50' },
      ...
      {effektivBudget > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          {nachtragGenehmigt > 0 && (
            <p className="text-xs text-emerald-600 mt-1">inkl. {fmtEur(nachtragGenehmigt)} genehmigte Nachträge</p>
          )}
          {nachtragEingereicht > 0 && (
            <p className="text-xs text-blue-500 mt-0.5">{fmtEur(nachtragEingereicht)} in Nachträgen eingereicht (noch nicht genehmigt)</p>
          )}
        </div>
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                {stundenProMa.map((m,i) => (
                  <div key={m.name} className="space-y-1">
                    ...
                    <div className="w-full bg-gray-100 rounded-full h-1.5">
                      <div className="h-1.5 rounded-full" style={{width:`${m.stunden/stundenProMa[0].stunden*100}%`, background:`hsl(${210+i*40},70%,45%)`}} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="col-span-full bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div><p className="text-xs text-gray-400">Frist</p><p className="font-medium text-gray-700">{fmtDate(bs.enddatum)}</p></div>
            </div>
            {bs.beschreibung && <p className="text-sm text-gray-600 mt-3 p-3 bg-gray-50 rounded-xl whitespace-pre-line">{bs.beschreibung}</p>}
          </div>
        </div>
      )}
            ...
            {sw.map((w: any) => (
              <div key={w.id} className="flex items-center gap-4 px-5 py-3">
                ...
                <div className="text-right flex-shrink-0">
                  <p className="font-mono font-bold text-gray-700">{w.stunden}h</p>
                  <p className="text-xs text-gray-400">{fmtEur(Number(w.stunden) * Number(w.employees?.stundensatz ?? STUNDEN_SATZ))}</p>
                </div>
                <div className="flex gap-1 flex-shrink-0">
                  <button onClick={() => { setStundenForm({ mitarbeiter_id: w.mitarbeiter_id, datum: w.datum, stunden: String(w.stunden), beschreibung: w.beschreibung||'' }); setEditStunden(w); setStundenDialog(true); }} className="p-1.5 rounded-lg hover:bg-gray-100">
                    <Pencil className="h-3.5 w-3.5 text-gray-400" />
                  </button>
                  <button onClick={() => { if(confirm('Stunden-Eintrag löschen?')) deleteStunden.mutate(w.id); }} className="p-1.5 rounded-lg hover:bg-red-50">
      ...
      {tab === 'Material' && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
          ...
          <div className="divide-y divide-gray-50">
            {mat.length === 0 && <p className="text-gray-300 text-center py-10 text-sm">Noch kein Material</p>}
            {mat.map((m: any) => (
              <div key={m.id} className="flex items-center gap-4 px-5 py-3">
                ...
                <div className="flex gap-1 flex-shrink-0">
                  <button onClick={() => { setMatForm({ bezeichnung:m.bezeichnung, menge:String(m.menge), einheit:m.einheit, einzelpreis:String(m.einzelpreis), gesamtpreis:String(m.gesamtpreis), status:m.status, datum:m.datum||'' }); setEditMaterial(m); setMaterialDialog(true); }} className="p-1.5 rounded-lg hover:bg-gray-100">
                    <Pencil className="h-3.5 w-3.5 text-gray-400" />
                  </button>
                  <button onClick={() => { if(confirm('Material löschen?')) deleteMat.mutate(m.id); }} className="p-1.5 rounded-lg hover:bg-red-50">
                    <Trash2 className="h-3.5 w-3.5 text-gray-400 hover:text-red-500" />
                  </button>
                </div>
              </div>
      ...
      {tab === 'Nachträge' && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: 'Gesamt beantragt', value: nach.reduce((s,n)=>s+Number(n.betrag??0),0), color: 'text-gray-700' },
          ...
          {nach.map((n: any) => {
            const ns = NACH_STATUS.find(s => s.value === n.status);
            return (
              <div key={n.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <button onClick={() => { setNachForm({ titel:n.titel, beschreibung:n.beschreibung||'', betrag:String(n.betrag), status:n.status, datum:n.datum||'', begruendung:n.begruendung||'' }); setEditNachtrag(n); setNachtragDialog(true); }} className="p-1.5 rounded-lg hover:bg-gray-100">
                      <Pencil className="h-3.5 w-3.5 text-gray-400" />
                    </button>
                    <button onClick={() => { if(confirm('Nachtrag löschen?')) deleteNach.mutate(n.id); }} className="p-1.5 rounded-lg hover:bg-red-50">
                      <Trash2 className="h-3.5 w-3.5 text-gray-400 hover:text-red-500" />
                    </button>
                  </div>
                </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><Label>Stunden *</Label><Input placeholder="z.B. 7.5 oder 7,5" value={stundenForm.stunden} onChange={e => setStundenForm((f: any) => ({...f, stunden: e.target.value}))} /></div>
            </div>
            <div><Label>Beschreibung</Label><Input placeholder="Was wurde gemacht?" value={stundenForm.beschreibung} onChange={e => setStundenForm((f: any) => ({...f, beschreibung: e.target.value}))} /></div>
